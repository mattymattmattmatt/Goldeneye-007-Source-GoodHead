...
// Orange Box / Source SDK 2007 CViewSetup (hl2sdk-orangebox/public/view_shared.h)
class CViewSetup
{
public:
	int			x;
	...
	// If GE:S is slightly newer than 2007, extra tail fields exist.
	// Keep this object larger than the engine's so copies stay in-bounds.
	char		_tail[256];
};
...
public:
	...
	// Do not read L4D2-sized fields off this object — GE:S player layout differs.
};
...
// WARNING -- UNVERIFIED VTABLE.
// In the retail Source SDK, vgui's IInput derives from IBaseInterface, which
// declares a virtual destructor. Under MSVC that destructor occupies vtable
// slot 0, so every method below would be off by one against the real
// vgui2.dll layout. Calling a wrong __thiscall slot does not raise an access
// violation -- it mismatches argument counts and corrupts the stack, which is
// why wrapping these calls in SEH never caught anything and the process died a
// few frames later instead. That is the best fit for the 2026-08-28 crash that
// happened moments after the main menu appeared.
//
// Menu input therefore defaults to posting real Win32 messages to the
// "Valve001" window (VR::m_MenuUseWin32), which is layout-independent. If you
// want to revisit this path, set MenuInputVguiInternal=true in config.txt and
// try adding a `virtual void dtor_placeholder() = 0;` as the first entry.
class IInput
{
public:
	virtual void SetMouseFocus(unsigned int panel) = 0;
	virtual void SetMouseCapture(unsigned int panel) = 0;
	virtual void GetKeyCodeText(ButtonCode_t code, char *buf, int buflen) = 0;
	virtual unsigned int GetFocus() = 0;
	virtual unsigned int GetMouseOver() = 0;
	virtual void SetCursorPos(int x, int y) = 0;
	virtual void GetCursorPos(int &x, int &y) = 0;
	virtual bool WasMousePressed(ButtonCode_t code) = 0;
	virtual bool WasMouseDoublePressed(ButtonCode_t code) = 0;
	virtual bool IsMouseDown(ButtonCode_t code) = 0;
	virtual void SetCursorOveride(void *cursor) = 0;
	virtual void *GetCursorOveride() = 0;
	virtual bool WasMouseReleased(ButtonCode_t code) = 0;
	virtual bool WasKeyPressed(ButtonCode_t code) = 0;
	virtual bool IsKeyDown(ButtonCode_t code) = 0;
	virtual bool WasKeyTyped(ButtonCode_t code) = 0;
	virtual bool WasKeyReleased(ButtonCode_t code) = 0;
	virtual unsigned int GetAppModalSurface() = 0;
	virtual void SetAppModalSurface(unsigned int panel) = 0;
	...
	virtual void InternalKeyCodeReleased(ButtonCode_t code) = 0;
};
